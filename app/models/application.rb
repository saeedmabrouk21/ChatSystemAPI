class Application < ApplicationRecord
  ...
  def generate_chat_number
    begin
      Redis.current.watch("application:#{self.id}:chats_max_number") do
        current_count = Redis.current.get("application:#{self.id}:chats_max_number").to_i
        new_count = current_count + 1

        # Use MULTI to start a transaction and execute the set atomically
        Redis.current.multi do
          Redis.current.set("application:#{self.id}:chats_max_number", new_count)
        end
